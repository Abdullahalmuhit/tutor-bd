...
<?php
				if (count($locations)>2)
				{
					...
					<?php
						$location_name = (strlen($location->location) > 20)?substr($location->location, 0, 18) . '..':$location->location;
						echo "<input type='checkbox' class='locations styled' id='locations_".$location->id."' name='locations[]' value='".$location->id."'>";
					?>
					<label for="locations_<?=$location->id?>"  class="input-label-checkbox" data-toggle='tooltip' title='<?php echo $location->location; ?>'>
						<?php echo $location_name; ?>
					</label>
                    </div>
