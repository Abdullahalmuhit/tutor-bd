                        ...
                        <div class="uk-container uk-container-expand" style="padding: 0 10px">
                            <div uk-navbar>
                                <ul class="uk-nav" style="float: right">
                                  <li style="cursor: pointer"><a style="color: #fff; cursor: pointer" href="<?php echo base_url('tutor/verify_invoice_list'); ?>"><button class="uk-badge uk-badge-success" style="padding: 6px 12px 6px 12px; margin: 4px 0 4px 0; background: #34495e; cursor: pointer">VERIFICATION PAYMENT INVOICE</button></a></li>
                                </ul>
                            </div>
                        </div>
                    </nav>
                ...
                <div class="uk-width-large-7-10">
                  ...
                  <?php else: ?>
                      <div class="md-card md-card-single main-print" id="invoice">
                            <?php if($this->session->flashdata('errormsg')) echo '<div class="uk-alert-warning" style="color:#fff; padding-left:20px;">'.$this->session->flashdata('errormsg').'</div>'; ?>
                ...
                <div class="uk-width-large-3-10 hidden-print uk-visible-large">
                    <div class="md-list-outside-wrapper">
                        <ul class="md-list md-list-outside invoices_list" id="invoices_list">
                            <!-- <button class="btn btn-warning btn-xs" type="button" onclick="location.href='<?php echo base_url('tutor/verify_invoice_list'); ?>';">Verification Payment Invoice</button> -->
                            <li class="heading_list">Tuition Matching Invoice List</li>

                            <?php foreach ($invoice_lists as $invoice_list) { //echo '<pre>'; print_r($invoice_list); exit; ?>
                                <li>
                                    <a href="#" class="md-list-content" data-paid-status="<?php echo $invoice_list['status']; ?>" data-invoice-id="<?php echo $invoice_list['id']; ?>" data-job_id="<?php echo $invoice_list['job_id']; ?>" data-date="<?php echo date('d/m/Y', strtotime($invoice_list['created_at'])); ?>" data-amount="<?php echo $invoice_list['amount']; ?>" data-tutor-id="<?php echo $invoice_list['tutor_id'] ?>" data-tutor-name="<?php echo $invoice_list['tutor_name'] ?>" data-tutor-email="<?php echo $invoice_list['tutor_email'] ?>" data-tutor-mobile="<?php echo $invoice_list['tutor_mobile'] ?>" data-job-title="Need a tutor for <?php echo $invoice_list['sub_cat']; ?> student" data-payment-date="<?php echo date('d/m/Y', strtotime($invoice_list['updated_at'])); ?>" data-due-date="<?php if($invoice_list['due_date']): echo date('d/m/Y', strtotime($invoice_list['due_date'])); else: echo '--'; endif ?>" data-tutor-gender="<?php echo $invoice_list['tutor_gender']; ?>">
                                        ...
                                        <span class="uk-text-small uk-text-muted">Job ID - <?php echo $invoice_list['job_id']; ?>, <?php echo date('F j, Y', strtotime($invoice_list['created_at'])); ?></span>
                                    </a>
                                </li>
                            <?php } ?>
                        </ul>
...
<script id="invoice_template" type="text/x-handlebars-template">
    <div class="uk-grid" style="margin: 0 !important">
      ...
      <div class="uk-width-medium-1-2 uk-width-small-1-1 p-20 uk-text-right" style="padding-right: 20px !important">
        <a style="color: #fff; cursor: pointer" target="blank" href="<?php echo base_url('tutor/makeJobPayment/{{invoice_id.invoice_number}}') ?>" class="pay_bill" data-job_id="{{invoice_id.invoice_job_id}}"><button class="uk-badge uk-badge-success" style="cursor: pointer; padding: 4px 10px 4px 10px; margin-top: 10px">Pay Now</button></a>
    ...
    <div class="uk-grid" style="margin: 20px !important; padding: 0 !important">
      <div class="uk-width-2-2" style="margin: 0 !important; padding: 0 !important">
        <table class="uk-table">
          <tr>
            <td style="color: #0675c1; font-weight: bold; border-bottom-style: solid; border-bottom-color: #0675c1; padding-left: 0 !important">Job Summery</td>
            <td style="color: #0675c1; font-weight: bold; border-bottom-style: solid; border-bottom-color: #0675c1; padding-left: 0 !important; text-align: right">Tuition Matching Charge</td>
          </tr>
          <tr>
            <td style="padding-left: 0 !important">Job ID - {{ invoice_id.invoice_job_id }} <br>{{ invoice_id.invoice_job_title }}</td>
